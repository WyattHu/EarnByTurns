...
interface IPrizePool {
    ...
    event ControlledTokenAdded(IShare indexed token);
    ...
    event Deposited(
        ...
        address indexed to,
        IShare indexed token,
        uint256 amount
    );

    /// @dev Event emitted when interest is awarded to a winner
    event Awarded(address indexed winner, IShare indexed token, uint256 amount);
    ...
    /// @dev Event emitted when the Share is set
    event ShareSet(IShare indexed share);
    ...
    /**
     * @notice Read internal Share accounted balance.
    ...
    /**
     * @notice Read Share variable
     */
    function getShare() external view returns (IShare);
    ...
    function isControlled(IShare controlledToken) external view returns (bool);
    ...
    /// @notice Set prize pool Share.
    /// @param share Address of the Share to set.
    /// @return True if Share has been successfully set.
    function setShare(IShare share) external returns (bool);
