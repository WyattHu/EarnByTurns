...
library TwabLib {
    ...
    /**
      * @notice Sets max ring buffer length in the Account.twabs Observation list.
                As users transfer/mint/burn share new Observation checkpoints are
    ...
    */
    //账户的细节，余额， 下一个未初始化或可更新的数据槽，所有已初始化的槽
    ...
    //增加账户余额，同时更新一个新的twab，减少的function类似
    ...
    */
    ...
    //返回最早之前的twab，如果slot都初始化完了，环形缓冲区已经占用完，那最老的twab应该是下一个index的twab，如果下一个twab的时间为0，代表还未初始化，那么最老的是index为0的twab
    function oldestTwab(
        ObservationLib.Observation[MAX_CARDINALITY] storage _twabs,
        AccountDetails memory _accountDetails
    ...
    */
    //获取某个时间点的balance，如果，1，目标时间大于最新的时间，返回当前的balance，2，目标时间小于最老的时间，返回0，
    //3，目标时间在最新和最老中间，用二分法计算
    ...
    /** @dev    Binary search in _calculateTwab fails when searching out of bounds. Thus, before
      ...
      * @return accountDetails Updated Account.details struct
    */
    //计算twab，如果 1，目标时间大于最新的时间，则计算下一个新的twab，2， 目标时间等于最新twab时间，返回最新的twab，
    //3，如果目标时间等于最老的twab时间，则返回最老的twab，4，如果目标时间小于最老的twab时间，则返回0
    //5，如果目标时间在最老和最新的中间，使用二分法计算
     ...
     */
     //计算下一个twab，amount是之前的amount加上当前余额乘以存放的时间
    ...
    //生成下一个twab，如果当前时间和当前最新的twab的时间相同，则返回当前最新的，如果不同，则计算出新的twab，并把detail index往后推移
    function _nextTwab(
        ObservationLib.Observation[MAX_CARDINALITY] storage _twabs,
        AccountDetails memory _accountDetails,
        uint32 _currentTime
    ...
    //把账户细则里面的index往后移位，如果初始化的slot数量小于最大值，则+1
